...
ADMIT_FORM_KEYS = {
    "Window": {
        ...
        "Window RMS noise": ["win_rms", "win.rms_w", _gen_numeric_sql],
        "Window Signal to noise ratio (peak/rms)": [
            "win_snr",
            "win.snr_w",
            _gen_numeric_sql,
        ],
    ...
    "Lines": {
        "Spectral window ID(L)": [
            ...
            "lines.w_id",
            _gen_numeric_sql,
        ],  # user should never select on this
    ...
    "Sources": {
        ...
        "Search Region": [
            ...
            "sources.region",
            None,
        ],  # special case where we will parse internally
    },
    ...
    "Alma": {
        ...
        "Target Region": [
            ...
            "alma.region",
            None,
        ],  # LMT ONLY since no sources table -special case where we will parse internally
        "Total Integration time (s)": [
            "total_integration_time",
            "alma.t_total_exptime",
            _gen_numeric_sql,
        ],  # LMT Only
        # Public Date aka  End of proprietary period
        "Public Date": [
            "public_date",
            "alma.public_date",
        ...
        "Galactic (Degrees)": [
            "galactic",
            "alma.gal_longitude, alma.gal_latitude",
            _gen_pos_sql,
        ],
        "Source Velocity": [
            "s_velocity",
        ...
        # Polarization
        "Polarisation type (Single, Dual, Full)": [
            "polarisation_type",
            "alma.pol_states",
            _gen_pol_sql,
        ],
        # Observation
        "Line sensitivity (10 km/s) (mJy/beam)": [
            "line_sensitivity",
...
class ADMITClass(BaseQuery):
    ...
    def report_version(self):
        return (
            f"Database version: {self.database_version}. core.py version: {version()}"
        )
    ...
    def load_admit(self, admit_db):
        ...
        if os.path.exists(admit_db):
            ...
            self.c = sqlite3.connect(admit_db, check_same_thread=self.check_same_thread)
    ...
    def _set_keys(self):
        ...
        self.ktable = Table(
            [c1, c2, c3], names=("Keyword", "Description", "Database Table")
        )
    ...
    def query_rdz(self, *args, **kwargs):
        ...
        """
        if self.rdz_lines is None:
            self.rdz_lines = open(
                self.q + "/datasetid_ra_dec_redshift_resolvername.txt"
            ).readlines()
    ...
    def query_sql_async(self, *args, **kwargs):
        ...
        result = self._request(
            "POST", url=self.request_url, data=payload, timeout=self.timeout
        )
    ...
    def _parse_args(self, sql_query):
        ...
        payload = {"query": sql_query, "format": "fits"}
    ...
    def help(self):
        ...
        print(
            "   win:       all SPW's are stored here, with links back to the alma reference"
        )
                    ...
                    for attrib in attrib_category.values():
                        if constraint in attrib:
                            ...
                            if constraint == "alma.em_resolution":
                                ...
                                constraint == "region" or constraint == "target_region"
                            ):
                            ...
                            if "win.snr_w" in attrib_where:
                                attrib_where = attrib_where.replace(
                                    "win.snr_w", "( win.peak_w / win.rms_w )"
                                )
                            if attrib_where:
                                if where:
                                    where += " AND "
            ...
            if isinstance(interval, tuple):
                ...
                if int_min is None:
                    if int_max is None:
                        ...
                        result += "ALMA.OBSNUM = ALMA.OBSNUMLIST AND ALMA.MIN_OBSNUM <= {1}".format(
                            field, int_max
                        )
                    ...
                    result += (
                        "ALMA.MIN_OBSNUM >= {1} AND ALMA.MAX_OBSNUM <= {2}".format(
                            field, int_min, int_max
                        )
                    )
            else:
                result += "{}={}".format(field, interval)
...
def get_admit_datafile(result, **kwargs):
    ...
    fitsfile = commons.FileContainer(result, encoding="binary", **kwargs)
...
def find_data_url(result_page):
    ...
    result_relative_url_re = re.compile(
        r"Download the result file: " r'<a href="(\.\./tmp/.*?)">'
    )
